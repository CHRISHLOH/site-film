...
CREATE TABLE content_service.localized_fields (
                                                  ...
                                                  locale VARCHAR(5) NOT NULL,               -- 'en', 'ru', 'de'
                                                  field_name VARCHAR(50) NOT NULL,          -- 'name', 'description'
                                                  field_value TEXT NOT NULL,
                                                  UNIQUE (entity_type, entity_id, locale, field_name)
);

CREATE INDEX idx_localized_fields_entity
    ON content_service.localized_fields (entity_type, entity_id, locale);
...
CREATE INDEX IF NOT EXISTS idx_movies_rating ON content_service.content (average_rating DESC) WHERE deleted_at IS NULL;
...
CREATE TABLE content_service.franchise_content (
                                                   franchise_id BIGINT NOT NULL,
                                                   content_id BIGINT NOT NULL,
                                                   chronological_order SMALLINT NOT NULL, -- хронологический порядок
